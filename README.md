...
```text
用户输入 -> 命理/日历特征 -> 评分规则 -> V5 路线拓扑 -> Route Data -> 地图 UI
```
...
- 核心体验：输入生日、时辰、出生地、性别，生成「当下 -> 未来」的渐进式路线导航图。
- 路线拓扑：`0-12` V5 故事路线，首屏只展开 `1-2` 个下一步选择。
- 地图渲染：`assets/generated/life-road-atlas-v2/00_life_story_base_map_v12.svg` + 动态节点图标。
...
```text
http://127.0.0.1:4173/projectmystery/
```

API：

```text
GET  /projectmystery/api/health
GET  /projectmystery/api/context
POST /projectmystery/api/roadmap-copy
```
...
- `/api/roadmap-copy`：可选调用模型层，只做受控短文案改写。

Vercel API 在：

- [api/context.js](api/context.js)
- [api/health.js](api/health.js)
- [api/roadmap-copy.js](api/roadmap-copy.js)

Vercel 路由配置：

- [vercel.json](vercel.json)

默认模型层为阿里云百炼 OpenAI-compatible Chat Completions：
...
如果没有 `DASHSCOPE_API_KEY`，系统会使用本地模板，不影响核心路线生成。
上线配置见 [Project Mystery Live Deploy](docs/deploy/projectmystery_live.md)。
LLM 文案输入标准见 [人生路况 LLM 文案标准 v2](docs/logic/v2_llm_wording_standard.md)。
